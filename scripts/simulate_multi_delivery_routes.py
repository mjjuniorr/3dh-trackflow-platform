from __future__ import annotations

import argparse
import json
import math
import os
import random
import signal
import sys
import time
from dataclasses import dataclass
from datetime import datetime, timezone

from kafka import KafkaProducer


@dataclass(frozen=True)
class Point:
    lat: float
    lng: float


ROUTES: dict[str, list[Point]] = {
    "entregador_001": [
        Point(-3.094167, -60.021944),
        Point(-3.096250, -60.019650),
        Point(-3.099450, -60.016980),
        Point(-3.104040, -60.011800),
        Point(-3.107400, -60.014150),
        Point(-3.111300, -60.018500),
        Point(-3.119000, -60.021700),
    ],
    "entregador_002": [
        Point(-3.119000, -60.021700),
        Point(-3.116200, -60.026800),
        Point(-3.112400, -60.030200),
        Point(-3.105800, -60.034200),
        Point(-3.100500, -60.031400),
        Point(-3.094167, -60.021944),
    ],
    "entregador_003": [
        Point(-3.104040, -60.011800),
        Point(-3.101000, -60.009900),
        Point(-3.097800, -60.008700),
        Point(-3.093500, -60.010400),
        Point(-3.090200, -60.014900),
        Point(-3.088888, -60.022222),
    ],
    "entregador_004": [
        Point(-3.084783, -60.074603),
        Point(-3.087500, -60.068800),
        Point(-3.090800, -60.061200),
        Point(-3.094900, -60.053400),
        Point(-3.099700, -60.044200),
        Point(-3.104500, -60.036000),
    ],
    "entregador_005": [
        Point(-3.088888, -60.022222),
        Point(-3.091700, -60.024900),
        Point(-3.095600, -60.026100),
        Point(-3.101000, -60.024700),
        Point(-3.107800, -60.022900),
        Point(-3.119000, -60.021700),
    ],
}

running = True


def main() -> int:
    args = parse_args()
    install_signal_handlers()
    random.seed(args.seed)

    samples = max(2, int(args.duration_minutes * 60 / args.interval_seconds) + 1)
    devices = [device.strip() for device in args.devices.split(",") if device.strip()]
    unknown = [device for device in devices if device not in ROUTES]
    if unknown:
        raise ValueError(f"Dispositivos sem rota configurada: {', '.join(unknown)}")

    producer = KafkaProducer(
        bootstrap_servers=[args.broker],
        value_serializer=lambda value: json.dumps(value, ensure_ascii=False).encode("utf-8"),
        key_serializer=lambda value: value.encode("utf-8"),
        acks="all",
        retries=5,
        linger_ms=50,
    )

    print(
        f"Simulando {len(devices)} entregadores por {args.duration_minutes} min "
        f"em {args.topic} via {args.broker}. Intervalo: {args.interval_seconds}s",
        flush=True,
    )

    try:
        for index in range(samples):
            if not running:
                break

            progress = index / (samples - 1)
            timestamp = datetime.now(timezone.utc).isoformat()

            for device_position, device_id in enumerate(devices):
                route = ROUTES[device_id]
                route_progress = (progress + (device_position * args.route_offset)) % 1
                current = interpolate_route(route, route_progress)
                previous = interpolate_route(route, max(0.0, route_progress - (1 / (samples - 1))))
                heading = calculate_heading(previous, current)
                speed = max(0, args.base_speed + random.uniform(-args.speed_jitter, args.speed_jitter))
                battery = max(
                    args.end_battery,
                    round(args.start_battery - ((args.start_battery - args.end_battery) * progress) - device_position),
                )

                payload = {
                    "device_id": device_id,
                    "lat": round(current.lat, 6),
                    "lng": round(current.lng, 6),
                    "speed": round(speed, 1),
                    "heading": round(heading),
                    "battery": battery,
                    "accuracy": args.accuracy,
                    "timestamp": timestamp,
                }

                metadata = producer.send(args.topic, key=device_id, value=payload).get(timeout=15)
                print(
                    f"[{index + 1:03d}/{samples:03d}] {device_id} offset={metadata.offset} "
                    f"lat={payload['lat']} lng={payload['lng']} heading={payload['heading']} "
                    f"speed={payload['speed']} battery={payload['battery']}",
                    flush=True,
                )

            if index < samples - 1:
                time.sleep(args.interval_seconds)
    finally:
        producer.flush()
        producer.close()

    print("Simulacao multi-entregador finalizada.", flush=True)
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Simula varias rotas de entrega publicando no Kafka.")
    parser.add_argument("--broker", default=os.getenv("KAFKA_BROKER"))
    parser.add_argument("--topic", default=os.getenv("KAFKA_TOPIC", "rastreamento"))
    parser.add_argument(
        "--devices",
        default=os.getenv(
            "SIM_DEVICES",
            "entregador_001,entregador_002,entregador_003,entregador_004,entregador_005",
        ),
    )
    parser.add_argument("--duration-minutes", type=float, default=float(os.getenv("SIM_DURATION_MINUTES", "30")))
    parser.add_argument("--interval-seconds", type=float, default=float(os.getenv("SIM_INTERVAL_SECONDS", "5")))
    parser.add_argument("--base-speed", type=float, default=float(os.getenv("SIM_BASE_SPEED", "31")))
    parser.add_argument("--speed-jitter", type=float, default=float(os.getenv("SIM_SPEED_JITTER", "6")))
    parser.add_argument("--accuracy", type=float, default=float(os.getenv("SIM_ACCURACY", "8")))
    parser.add_argument("--start-battery", type=int, default=int(os.getenv("SIM_START_BATTERY", "94")))
    parser.add_argument("--end-battery", type=int, default=int(os.getenv("SIM_END_BATTERY", "84")))
    parser.add_argument("--route-offset", type=float, default=float(os.getenv("SIM_ROUTE_OFFSET", "0.11")))
    parser.add_argument("--seed", type=int, default=int(os.getenv("SIM_SEED", "3")))
    args = parser.parse_args()
    if not args.broker:
        parser.error("Informe --broker ou KAFKA_BROKER. Nao use broker Kafka publico sem VPN, tunel ou firewall restrito.")
    return args

def interpolate_route(route: list[Point], progress: float) -> Point:
    progress = min(max(progress, 0.0), 1.0)
    segment_count = len(route) - 1
    raw_position = progress * segment_count
    segment_index = min(int(raw_position), segment_count - 1)
    local_progress = raw_position - segment_index
    start = route[segment_index]
    end = route[segment_index + 1]
    return Point(
        lat=start.lat + ((end.lat - start.lat) * local_progress),
        lng=start.lng + ((end.lng - start.lng) * local_progress),
    )


def calculate_heading(start: Point, end: Point) -> float:
    lat1 = math.radians(start.lat)
    lat2 = math.radians(end.lat)
    delta_lng = math.radians(end.lng - start.lng)
    y = math.sin(delta_lng) * math.cos(lat2)
    x = (math.cos(lat1) * math.sin(lat2)) - (math.sin(lat1) * math.cos(lat2) * math.cos(delta_lng))
    bearing = math.degrees(math.atan2(y, x))
    return (bearing + 360) % 360


def install_signal_handlers() -> None:
    def stop(_signum, _frame) -> None:
        global running
        running = False

    signal.signal(signal.SIGINT, stop)
    signal.signal(signal.SIGTERM, stop)


if __name__ == "__main__":
    sys.exit(main())
